...
def main() -> None:
    """
    Run the Upstox OAuth flow and output the resulting token payload.
    
    Parses command-line options --no-browser (print auth URL instead of opening a browser)
    and --timeout (seconds to wait for the authorization callback). Loads Upstox
    configuration, executes the OAuth flow, prints the token payload as pretty-printed
    JSON to stdout, logs a reminder to store the access token for downstream services,
    and exits with status code 1 on failure.
    """
