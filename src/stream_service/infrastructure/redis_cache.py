...
class RedisLatestPriceCache(LatestPriceCache):
    ...
    def __init__(self, config: RedisConfig) -> None:
        """
        Initialize a Redis-backed latest-price cache.
        
        Parameters:
            config (RedisConfig): Configuration containing the Redis connection URL and TTL for cached entries.
        """
        self.config = config
    ...
    def upsert_price(self, tick: Tick) -> None:
        """
        Store or update the latest price snapshot for the given instrument and persist it with the configured TTL.
        
        Parameters:
        	tick (Tick): Tick containing `instrument_id`, `price`, and `timestamp` to be cached.
        """
